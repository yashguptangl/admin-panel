...
export default function NotVerifiedRoom() {
    const router = useRouter();
    const [notVerified, setNotVerified] = useState<Room[]>([]);
    interface Room {
        id: number;
        city: string;
        townSector: string;
    ...
    useEffect(() => {
        const fetchNotVerifiedRooms = async () => {
            try {
                const response = await axios.get("http://localhost:3001/api/v1/admin/not-verified-room", {
                    method: "GET",
                    headers: {
        ...
        <>
            ...
            <div className="flex">
                ...
                <div className="flex-grow p-6">
                    {/* Agent Table */}
                    <h2 className="text-xl font-semibold mb-4 text-center">Not Verified Room List</h2>
                                ...
                                {notVerified.length === 0 ? (
                                    ...
                                    notVerified.map((room, index) => (
                                        <tr key={room.id} className="hover:bg-gray-100">
                                            ...
                                            <td className="border p-2">{room.MinPrice}</td>
                                            <td className="border p-2">{room.MaxPrice}</td>
                                            <td className="border p-2">{room.location}</td>
                                            <td className="border p-2">{room.listingShowNo}</td>

                                            <td className="border p-2 text-center">
                                                <button
                                                    className="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600"
                                                    onClick={() => {
                                                        router.push(`/not-verified-room/${room.id}`);
                                                    }}
                                                >
                                                    Open
                                                </button>
