...
const PropertyDetailsPage: React.FC<{ property: PropertyDetails , owner?:Owner }> = ({ property , owner }) => {
    ...
    // Helper to get KYC docs
    const getKycDocs = () =>
        Array.isArray(property.kycDocuments) ? property.kycDocuments : [];

    // Helper to get self verified images
    const getSelfVerifiedImages = () =>
        Array.isArray(property.selfVerifiedImages) ? property.selfVerifiedImages : [];
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        <div className="bg-white rounded-xl shadow-md overflow-hidden">
                            <div className="p-6">
                                ...
                                {/* KYC Documents */}
                                {getKycDocs().length > 0 && renderImageGallery(getKycDocs(), 'KYC Documents')}

                                {/* Self Verified Images */}
                                {getSelfVerifiedImages().length > 0 && renderImageGallery(getSelfVerifiedImages(), 'Self Verified Images')}
                                <button
                                    type="submit"
                                    onClick={handleApproveVerification}
                                    className="flex justi px-6 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors shadow-sm"
                                >
